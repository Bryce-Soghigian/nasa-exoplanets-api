...
function searchDatabaseForColumn(column_name,table){
    return db.select(column_name).from(table)
}
function searchDatabaseForItem(column_name,table,id){
    return db.where({id}).select(column_name).from(table)
}